...
const getMarketPlaceItems = (type, wants) =>
  Promise.all(wants.map(want => marketplaceSearch(type, want.id)));

const getSaleItemsByMaster = getMarketPlaceItems.bind(null, 'master');

const getSaleItemsByRelease = getMarketPlaceItems.bind(null, 'release');

const getSaleItemsByLabel = getMarketPlaceItems.bind(null, 'label');
...
getSaleItemsByRelease(sampleWantListObject).then(json => console.log(JSON.stringify(json)));

module.exports = {
  makeWantsArray,
  getSaleItemsByMaster,
  getSaleItemsByRelease,
  getSaleItemsByLabel
};
