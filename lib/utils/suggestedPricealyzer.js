...
const makeNumber = (price, convertedPrice, shipping) => {
  const convertedPriceExtracted = string.match(/\d+[.,]\d+[.,]?(\d+)?/);
  if(convertedPriceExtracted)
    return +number[0];
  return NaN;
};

const makeItemOnlyPrice = ({ price, converted_price: convertedPrice, shipping }) => { //saleItem
  return makeNumber(price, convertedPrice, shipping) || makeNumber(price);
};
