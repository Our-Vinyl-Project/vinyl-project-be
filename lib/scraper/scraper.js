...
util.processBody = function(body, params){
  ...
  if($(!'shortcut_navigable')) listing.push({ title: $('title').text().slice(0, -19), thumbnail: $('.thumbnail_center img').eq(0).attr('src') });
  else {
...
util.processPaginationResponse = function(obj, params){
  ...
  var perPage = (params.pagination && params.pagination.per_page) || 250;
